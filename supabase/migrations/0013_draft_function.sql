-- 0013_draft_function.sql
-- Atomic draft-save sibling to submit_ballot. Used by the autosave path on
-- the ranking page. Differences from submit_ballot:
--   * does NOT set submitted_at — ballot stays in draft state
--   * empty array is permitted (means "I cleared all my rankings") —
--     deletes every row in `rankings` for the caller's ballot, leaves
--     the ballot row in place
--   * does NOT write to audit_log — drafts are intermediate state and
--     we don't want to drown the log on every drag-drop. Only
--     submit_ballot and unlock_ballot audit on the ballot lifecycle.
--
-- Auth, eligibility, validation, and the FOR UPDATE lock pattern are
-- identical to submit_ballot. Same error codes for parity at the action
-- layer.
