...
- `assign_topic(target_id uuid, topic_id integer)` — admin-only. Sets `topics.presenter_voter_id`, used for first-time assignment and reassignment. Migration 0020 hardened the function: it now releases the student's previous topic (clearing FK + scheduled_for + all art_* fields) in the same transaction as the new assignment, so the unique-constraint violation that struck before is gone. Both source and destination are guarded against `presented_at IS NOT NULL` (raises `STUDENT_ALREADY_PRESENTED` and `TOPIC_ALREADY_PRESENTED` respectively). When the (target, topic) pair already matches the current state, the function short-circuits silently — no audit-log entry, no errors. Migration 0021 added a polls gate (`POLLS_LOCKED`) that refuses reassignment while polls are locked, the deadline has passed, or a tally is cached — reassigning after vote-lock would silently substitute a presenter for committed ballots. Reopening polls clears all three flags (per 0019), so the gate un-blocks itself automatically. Storage cleanup for both prefixes happens in the action layer.
